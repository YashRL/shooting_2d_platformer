...
class MetalMovingPlatform(pygame.sprite.Sprite):
    def __init__(self, x, y, properties):
        super().__init__()
        self.tile_size = properties.get('tile_size', 36)
        self.speed = properties.get('speed', 2.0)
        ...
        # Load and combine 3 images: start, middle, end
        self.image = pygame.Surface((self.tile_size * 3, self.tile_size), pygame.SRCALPHA)
        try:
            start_img = pygame.image.load("Assets/PNG/Tiles/Tiles/Moving_Platform/start.png").convert_alpha()
            mid_img = pygame.image.load("Assets/PNG/Tiles/Tiles/Moving_Platform/middle.png").convert_alpha()
            end_img = pygame.image.load("Assets/PNG/Tiles/Tiles/Moving_Platform/end.png").convert_alpha()
            
            start_img = pygame.transform.scale(start_img, (self.tile_size, self.tile_size))
            mid_img = pygame.transform.scale(mid_img, (self.tile_size, self.tile_size))
            end_img = pygame.transform.scale(end_img, (self.tile_size, self.tile_size))
            
            self.image.blit(start_img, (0, 0))
            self.image.blit(mid_img, (self.tile_size, 0))
            self.image.blit(end_img, (self.tile_size * 2, 0))
        except Exception as e:
            print(f"Error loading metal platform assets: {e}")
